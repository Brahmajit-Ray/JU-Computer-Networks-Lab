...
void sender(){
	...
	frame ack_frame;
	ack_frame=receive_frame();
	if(ack_frame.kind==acknowledgement){
		printf("Ack Frame received for packet-number %d\n",ack_frame.ack);
	}
}
...
int receiver(){
	...
	frame ack_frame=make_frame(acknowledgement,0,ack_no);
	send_frame(ack_frame);
}

int main(){
	int track=0;
	printf("Stop_N_Wait ARQ(Noiseless) Simulator for upto 5 packets\n");
	while(track<5){
		sender();
		track++;
	}
